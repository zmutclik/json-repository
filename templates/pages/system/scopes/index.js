...
$(document).ready(function () {
    oTable = $('#table_').DataTable({
        serverSide: true,
        ajax: {
            "url": '/page/scopes/{{clientId}}/{{sessionId}}/datatables', "contentType": "application/json", "type": "POST",
            "data": function (d) {
    ...
    $("#btnTambah").on("click", function () {
        window.location.href = '/page/scopes/{{clientId}}/{{sessionId}}/add';
    });

    $("#table_").on("click", '.btnEdit', function () {
        window.location.href = '/page/scopes/{{clientId}}/{{sessionId}}/' + $(this).parents('tr').attr('id');
    });
    ...
    $("#table_").on("click", '.btnDelete', function () {
        ...
        Swal.fire({
            title: 'Apakah anda YAKIN ingin menghapus SCOPE "' + nm + '"?',
