...
$(document).ready(function () {
    oTable = $('#table_').DataTable({
        serverSide: true,
        ajax: {
            "url": '/page/menu/{{clientId}}/{{sessionId}}/datatables', "contentType": "application/json", "type": "POST",
            "data": function (d) {
        ...
        columnDefs: [{
            ...
            render: function (data, type, row, meta) {
                ...
                btnhtml += "<button type=\"button\" class=\"btn btn-primary btnDetail\"><i class=\"lni lni-eye\"></i></button>";
                btnhtml += "</div>"
                return btnhtml;
            }
        }],
    ...
    $("#btnTambah").on("click", function () {
        window.location.href = '/page/menu/{{clientId}}/{{sessionId}}/add';
    });

    $("#table_").on("click", '.btnEdit', function () {
        window.location.href = '/page/menu/{{clientId}}/{{sessionId}}/' + $(this).parents('tr').attr('id');
    });

    $("#table_").on("click", '.btnDetail', function () {
        window.location.href = '/page/menu/detail/{{clientId}}/{{sessionId}}/' + $(this).parents('tr').attr('id');
    });
